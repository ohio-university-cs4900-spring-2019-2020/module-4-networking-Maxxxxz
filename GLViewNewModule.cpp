...
GLViewNewModule::~GLViewNewModule()
{
   //Implicitly calls GLView::~GLView()
	...
	//netmsg so we have access to those functions when compiler optimizes code
	Module5NetMsg x;
...
void GLViewNewModule::onKeyDown( const SDL_KeyboardEvent& key )
{
   ...
   if( key.keysym.sym == SDLK_1 )
   {
	   ...
	   Module5NetMsg msg;
	   /*
	   WO* wo = this->worldLst->at(worldLst->size()-1);
	   NetMessengerStreamBuffer* in = new NetMessengerStreamBuffer(true, 16);
	   unsigned int id = wo->getID();
	   *in << id;
	   *in << 5.0f;
	   *in << 10.0f;
	   *in << 5.0f;
...
WorldContainer* Aftr::GLViewNewModule::getWorldContainer()
{
	return worldLst;
}
...
void Aftr::GLViewNewModule::loadMap()
{
   ...
   //netLst->push_back( wo );
   /*
   netObj = WO::New(shinyRedPlasticCube, { 1, 1, 1 });
   netObj->setLabel("Net Object");
   netObj->setPosition({ 5, 5, 5 });
   worldLst->push_back(netObj);
   */
...
void GLViewNewModule::createRandomBoxes()
{
	float rx, ry, rz, rsx, rsy, rsz;


	srand(numboxes);
	rsx = rand() % 5 + ((rand() % 10) / 10.0);
	rsy = rand() % 5 + ((rand() % 10) / 10.0);
	rsz = rand() % 5 + ((rand() % 10) / 10.0);

	rx = rand() % 25 + ((rand() % 25) / 10.0);
	ry = rand() % 25 + ((rand() % 25) / 10.0);
	rz = rand() % 25 + ((rand() % 25) / 10.0);

	WO* wo = WO::New((ManagerEnvironmentConfiguration::getSMM() + "/models/cube4x4x4redShinyPlastic_pp.wrl"), { rsx, rsy, rsz });
	worldLst->push_back(wo);
	std::cout << wo << std::endl;
	wo->setPosition({ rx, ry, rz });
	numboxes++;
}
